...
app.config["JWT_SECRET_KEY"]="rff3e435fefe4f4fJHjh7g7Ghh7h7HUh7HjG521HHGgHg"
jwt = JWTManager(app)
...
def getCandidato(id):
    headers = {"Content-Type": "application/json; charset=utf-8"}
    url = dataConfig["url-backend-votaciones"] + '/candidatos/'+id
    response = requests.get(url, headers=headers)
    json = response.json()
    return jsonify(json)
...
def asignarPartidoCandidato(id,id_partido):
    data = request.get_json()
    headers = {"Content-Type": "application/json; charset=utf-8"}
    url = dataConfig["url-backend-votaciones"] + '/candidatos/'+id ,'/partidos/'+id_partido
    response = requests.put(url, headers=headers, json=data)
    json = response.json()
    return jsonify(json)
...
@app.route("/permisos/{id}", methods=['GET'])
def getPermiso(id):
    headers = {"Content-Type": "application/json; charset=utf-8"}
...
@app.route("/permisos/{id}",methods=['PUT'])
...
@app.route("/permisos/{id}",methods=['DELETE'])
...
@app.route("/permisos-roles/rol/{id_rol}/permiso/{id_permiso}",methods=['POST'])
...
@app.route("/permisos-roles/{id}", methods=['GET'])
...
@app.route("/permisos-roles/rol/{id_rol}/permiso/{id_permiso}",methods=['PUT'])
def modificarPermisosRoles(id_rol,id_permiso):
    data = request.get_json()
    headers = {"Content-Type": "application/json; charset=utf-8"}
    url = dataConfig["url-backend-security"] + '/permisos-roles','/rol/'+id_rol,'/permiso/'+id_permiso
    response = requests.put(url, headers=headers, json=data)
...
@app.route("/permisos-roles/{id}",methods=['DELETE'])
